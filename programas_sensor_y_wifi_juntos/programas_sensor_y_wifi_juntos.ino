...
// WiFi
const char* ssid = "ALUMNOS_CICLOS";//wifi clase :"ALUMNOS_CICLOS" MOVISTAR_D1EC
const char* password = "Ciclos2025sz?";//password clase :"Ciclos2025sz?" CDC574D8A95E6FDE69C1

// configuración  NTP (network time protocol)
const char* ntpServer = "es.pool.ntp.org";
const int32_t gmtOffset_sec = 3600;
const int32_t daylightOffset_sec = 0;//horario de verano 
...
void setup() {
  ...
  pinMode (led_caution_yellow,OUTPUT); 
  pinMode (led_clear_green, OUTPUT);
...
//obtención de hora y formateo en el formato correcto legible y la muestra.
  if (getLocalTime(&timeinfo)) {
    ...
    mqtt_client.publish("sensor/hora", formattedTime,distante);
